...
class sweetpicker_openmanipulator_coordinator:
    ...
    def pose_callback(self, msg: Pose) -> None:
        ...
        try:
            ...
            # Home coordinates in direction of the x-axis
            target_home = SetKinematicsPoseRequest()

            target_home.kinematics_pose.pose.position.x = float(
                format(float(0.15), ".3f")
            )
            target_home.kinematics_pose.pose.position.y = float(
                format(float(0.0), ".3f")
            )
            target_home.kinematics_pose.pose.position.z = float(
                format(float(0.08), ".3f")
            )
...
#                    target_home.kinematics_pose.pose.position
#                )
                rospy.loginfo(
